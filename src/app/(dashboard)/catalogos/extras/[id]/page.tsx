...
export default function EditarExtraPage() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          duracionEstimada: extra.duracionEstimada?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors({ submit: data.error || 'Error al actualizar extra' })
      }
    } catch (error) {
      setErrors({ submit: 'Error al actualizar extra' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Editar Extra</h1>
      </div>
