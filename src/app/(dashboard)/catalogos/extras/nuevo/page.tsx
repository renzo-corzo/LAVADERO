...
export default function NuevoExtraPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        router.push('/catalogos/extras')
      } else {
        const data = await response.json()
        setErrors({ submit: data.error || 'Error al crear extra' })
      }
    } catch (error) {
      setErrors({ submit: 'Error al crear extra' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900">Nuevo Extra</h1>
      </div>
            ...
            {puedeElegir && (
              <div className="md:col-span-2">
                ...
                <p className="mt-1 text-xs text-muted">
                  Elegí una sucursal si este extra (o su precio) es exclusivo de esa sede.
                </p>
              </div>
            )}
