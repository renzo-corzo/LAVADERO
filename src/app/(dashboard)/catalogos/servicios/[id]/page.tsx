...
export default function EditarServicioPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrors({ submit: data.error || 'Error al actualizar servicio' })
      }
    } catch (error) {
      setErrors({ submit: 'Error al actualizar servicio' })
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-ink">Editar Servicio</h1>
      </div>
